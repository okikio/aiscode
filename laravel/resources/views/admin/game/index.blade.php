            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        <a href="{{route('admin.game.create')}}" class="btn btn-outline-primary float-right"><i class="fa fa-plus"></i> Add Game </a>
                    </div>
                    <!-- /.card-header -->
                    <div class="card-body">
                        {!! $html->table(['class' => 'table table-bordered table-hover table-striped','id' => 'game-table'], true) !!}
                    </div>
                    <!-- /.card-body -->
                </div>
                <!-- /.card -->
...
@push('scripts')
            ...
            if (result.isConfirmed) {
                ...
                success:function(msg){
                    ...
                    if(msg.status == 'success'){
                        ...
                        window.LaravelDataTables["game-table"].ajax.reload();
                    }else{
    ...
    function changeStatus(status, id){
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                url:"{{route('admin.game.changeStatus')}}",
                type: 'post',
                headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
